   ...
   function onPartnerMessage(message) {
    ...
    if (_sessionDialogue.length === 0) {
        ...
        listHtml = _sessionDialogue.map(m => {
            // ---- 拍一拍类型 ----
            if (m.type === 'poke') {
                return `<div style="display:flex;justify-content:center;align-items:center;padding:6px 0;">
...
// 暴露给全局（让主页和 core.js 能调用）
window.openCompanionHistory = openCompanionHistory;
