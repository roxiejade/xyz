...
(function () {
    ...
    // ─── 陪伴对话气泡 ──────────────────────────────────────────────────────
// 本次陪伴中梦角说过的所有话（退出时清空）
// 每条消息格式：{ text, image, sender, replyTo, replyToSender, type }
let _sessionDialogue = [];
    ...
    // 获取引用信息
    const replyInfo = window._companionReplyContext || {};
   ...
   function onUserMessage(message) {
    ...
    if (_sessionDialogue.length === 0) {
        ...
        listHtml = _sessionDialogue.map(m => {
            ...
            if (m.type === 'poke') {
                return `
                    <div style="display:flex;justify-content:center;align-items:center;padding:6px 0;">
                        <div style="font-size:12px;color:rgba(255,255,255,0.45);font-style:italic;letter-spacing:0.5px;background:rgba(255,255,255,0.06);padding:4px 14px;border-radius:20px;">
                            ✦ ${escapeHtml(m.text)} ✦
                        </div>
                    </div>
                `;
            ...
            let replyHtml = '';
            if (m.replyTo) {
                const senderName = m.replyToSender || '梦角';
                const replyPreview = m.replyTo.substring(0, 40) + (m.replyTo.length > 40 ? '…' : '');
                replyHtml = `
                    <div style="font-size:10.5px;color:rgba(255,255,255,0.35);margin-bottom:4px;padding:3px 8px;border-left:2px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.04);border-radius:0 6px 6px 0;line-height:1.4;">
                        ↳ 回复 ${escapeHtml(senderName)}：${escapeHtml(replyPreview)}
                    </div>
                `;
            }
            ...
            return `
                <div class="${itemClass}">
                    <div class="companion-bubble-avatar">${avatarHtml}</div>
                    <div class="companion-bubble-content" style="background:${isUser ? 'rgba(var(--accent-color-rgb,197,164,126),0.4)' : 'rgba(255,255,255,0.12)'};">
                        ${replyHtml}
                        ${escapeHtml(m.text || '')}
                    </div>
                </div>
    ...
    // 关闭按钮
    modal.querySelector('.companion-history-close').addEventListener('click', function() {
        modal.classList.remove('active');
        setTimeout(function() { if (modal.isConnected) modal.remove(); }, 300);
...
// ── 监听引用回复，记录上下文 ──
document.addEventListener('click', function(e) {
    const replyBtn = e.target.closest('.reply-indicator, [data-action="reply"]');
    if (replyBtn) {
        const textEl = replyBtn.querySelector('.reply-indicator-text');
        const text = textEl ? textEl.textContent : '';
        const sender = replyBtn.dataset.replySender || '梦角';
        window._companionReplyContext = {
            replyTo: text,
            replyToSender: sender
        };
        setTimeout(function() {
            window._companionReplyContext = null;
        }, 3000);
    }
});
    }
...
// ── 全局拍一拍记录函数（供主聊天调用） ──
window._recordCompanionPoke = function(text) {
