...
if (message.sender !== 'user' && message.type === 'normal' && typeof window._onPartnerMessage === 'function') {
    try {
        // 把引用信息附加到 message 上，让陪伴页能显示回复引用
        if (window._companionReplyContext) {
            message.replyTo = window._companionReplyContext.replyTo || null;
            message.replyToSender = window._companionReplyContext.replyToSender || null;
        }
        window._onPartnerMessage(message);
    } catch (e) { console.warn('[onPartnerMessage]', e); }
}
...
if (message.sender === 'user' && message.type === 'normal' && typeof window._onUserMessage === 'function') {
    try {
        ...
        window._onUserMessage(message);
    } catch (e) { console.warn('[onUserMessage]', e); }
}
};
    ...
    if (replyIndicator) {
        const replyTextEl = replyIndicator.querySelector('.reply-indicator-text');
        const replySenderEl = replyIndicator.querySelector('.reply-indicator-sender');
        window._companionReplyContext = {
            replyTo: replyTextEl ? replyTextEl.textContent : '',
            replyToSender: replySenderEl ? replySenderEl.textContent : '梦角'
        };
    } else {
